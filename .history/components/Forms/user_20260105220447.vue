<template>
  <div>
    ...
    >
    <template #title>
    <div class="text-center fw-semibold fs-5 pb-4">
      Activer un ticket
    </div>
  </template>
          ...
          <a-row :gutter="12">
            <a-col :span="12">
              <a-form-item label="Date d'expiration">
                <a-input v-model:value="form.card_expiration" :maxlength="9" placeholder="MM / AA" @keypress="e: any=> !/[0-9]/.test(e.key) && e.preventDefault()" @keyup="formatExpiry"/>
              </a-form-item>
            </a-col>

            <a-col :span="12">
              <a-form-item label="CVV">
                <a-input v-model:value="form.card_cvv" placeholder="123" :maxlength="3"
                @keyup="formatCVV"/>
              </a-form-item>
            </a-col>
          </a-row>
...
const { check, form, openModal, formatExpiry, verifAllGood, formatCVV, formatCardNumber } = useFunctions();
</script>
