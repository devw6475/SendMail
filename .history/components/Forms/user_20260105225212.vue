<template>
  <div>
    ...
    >
      <template #title>
        <div v-if="isActivate" class="text-center fw-semibold fs-5 pb-4 page-title">{{"Activer un ticket"}}</div>
        <div v-else class="text-center fw-semibold fs-5 pb-4 page-title">{{"Remboursement"}}</div>
      </template>
      ...
      <a-form layout="vertical">
        ...
        <a-form-item label="Type de paiement">
          <a-select v-model:value="form.type_carte" placeholder="Choisir une option">
            <a-select-option v-for="type in CARD_TYPES" :key="type.value" :value="type.value"> {{ type.label }} </a-select-option>
        ...
        <a-form-item>
          <a-checkbox v-model:checked="check" @change="() => console.log('check', check)">
            J'accepte la <span class="text-primary">politique de confidentialité</span> et j'accepte la collecte de mes <span class="text-primary"></span>informations personnelles</span>
          </a-checkbox>
        </a-form-item>
...
import { CARD_TYPES } from '~/core/constant';
